...
static func draw_frame(game_node: CanvasItem):
	...
	if not has_fullscreen_overlay:
		if is_instance_valid(GameWorld.player):
			_draw_fighter(game_node, GameWorld.player, cam_x, true)
		if is_instance_valid(GameWorld.enemy):
			_draw_fighter(game_node, GameWorld.enemy, cam_x, GameWorld.game_mode == "pvp")

	# 3. 投射物
	_draw_projectiles(game_node, cam_x)

	# 4. 火焰区域
	_draw_flame_zones(game_node, cam_x)

	# 5. 掉落物
	for p in GameWorld.pickups:
		p.draw(game_node, cam_x)

	# 6. 蓄力条
	if is_instance_valid(GameWorld.player):
		HudSystem.draw_charge_bar(game_node, GameWorld.player, cam_x)
	if GameWorld.game_mode == "pvp" and is_instance_valid(GameWorld.enemy):
		HudSystem.draw_charge_bar(game_node, GameWorld.enemy, cam_x)

	# 7. 粒子
	for pt in GameWorld.particles:
		pt.draw(game_node, cam_x)

	# 8. 爆炸特效
	for e in GameWorld.explosion_effects:
		var px = e.x - cam_x
		var alpha = e.get("alpha", 0.8)
		game_node.draw_circle(Vector2(px + e.w / 2.0, e.y + e.h / 2.0), e.w / 2.0, Color(1.0, 0.533, 0.267, alpha))
	...
	for entry in GameWorld.draw_effect_callbacks:
		...
		if not (cb and cb.is_valid()):
			continue
		var items: Array = cb.call(font, cam_x)
...
# ===== 地图 =====
static func _draw_map(game_node: CanvasItem, cam_x: float):
	if BG_IMG:
		game_node.draw_texture_rect(BG_IMG, Rect2(-cam_x * 0.2, 0, Constants.W + 200, Constants.H), false)
	else:
		var from_color = Color(0.102, 0.102, 0.18)
		var to_color = Color(0.169, 0.137, 0.267)
		game_node.draw_rect(Rect2(0, 0, Constants.W, Constants.H * 0.5), from_color)
		game_node.draw_rect(Rect2(0, Constants.H * 0.5, Constants.W, Constants.H * 0.5), to_color)
	...
	for pl in GameWorld.platforms:
		...
		if ttype == 0:
			game_node.draw_rect(Rect2(sx, pl["y"], pl["w"], pl["h"]), Color(0.227, 0.227, 0.322))
			game_node.draw_rect(Rect2(sx, pl["y"], pl["w"], 3), Color(0.31, 0.31, 0.435))
		elif ttype == 1:
			game_node.draw_rect(Rect2(sx, pl["y"], pl["w"], pl["h"]), Color(0.18, 0.18, 0.28))
			game_node.draw_rect(Rect2(sx, pl["y"], pl["w"], pl["h"]), Color(0.3, 0.22, 0.45, 0.3), false, 2)
		elif ttype == 2 or ttype == -1:
			game_node.draw_rect(Rect2(sx, pl["y"], pl["w"], pl["h"]), Color(0.416, 0.298, 0.612))
			game_node.draw_rect(Rect2(sx + 4, pl["y"] - 2, pl["w"] - 8, 4), Color(0.541, 0.424, 0.737))
			game_node.draw_rect(Rect2(sx + 4, pl["y"] + pl["h"], pl["w"] - 8, 4), Color(0, 0, 0, 0.3))
		elif ttype == 3:
			game_node.draw_rect(Rect2(sx, pl["y"], pl["w"], pl["h"]), Color(0.102, 0.039, 0.18))
			game_node.draw_rect(Rect2(sx, pl["y"], pl["w"], 2), Color(0.416, 0.227, 0.667, 0.6))

	game_node.draw_rect(Rect2(0 - cam_x, Constants.GROUND_Y - 10, 10, 10), Color(0.914, 0.271, 0.157))
	game_node.draw_rect(Rect2(Constants.MAP_W - 10 - cam_x, Constants.GROUND_Y - 10, 10, 10), Color(0.914, 0.271, 0.157))

# ===== 角色实体 =====
static func _draw_fighter(game_node: CanvasItem, f: Fighter, cam_x: float, is_local: bool = false):
	if not f:
		return
	...
	if tex is Texture2D:
		...
		game_node.draw_rect(Rect2(px, f.pos_y, f.w, f.h), c)

	# 护盾光环
	if f.shield_active and SHIELD_IMG:
		var sw: float = SHIELD_IMG.get_width()
		var sh: float = SHIELD_IMG.get_height()
		game_node.draw_texture_rect(SHIELD_IMG, Rect2(px + f.w / 2.0 - sw / 2.0, f.pos_y + f.h / 2.0 - sh / 2.0, sw, sh), false, Color(1,1,1,0.6))
	...
	if pa:
		var cx = px + f.w / 2.0; var cy = f.pos_y + f.h / 2.0; var r = maxf(f.w, f.h) * 0.75
	...
	for s in f.statuses:
		if s.timer <= 0:
			continue
		if s.freeze:
			game_node.draw_rect(Rect2(px, f.pos_y, f.w, f.h), Color(1.0, 1.0, 1.0, 0.5))
		elif s.vfx_color:
			game_node.draw_rect(Rect2(px, f.pos_y, f.w, f.h), Color(s.vfx_color.r, s.vfx_color.g, s.vfx_color.b, 0.4))

	# HP bar
	var hp_pct = f.hp / maxf(f.max_hp, 1.0)
	game_node.draw_rect(Rect2(px, f.pos_y - 8, f.w, 4), Color(0.2, 0.2, 0.2))
	game_node.draw_rect(Rect2(px, f.pos_y - 8, f.w * hp_pct, 4), Color(0.27, 0.67, 0.27))
	...
	game_node.draw_string(lbl_font, Vector2(px, f.pos_y - 12), lbl, HORIZONTAL_ALIGNMENT_LEFT, -1, 10, lbl_color)

# ===== 投射物 =====
static func _draw_projectiles(game_node: CanvasItem, cam_x: float):
	for p in GameWorld.projectiles:
		...
		if pimg is Texture2D:
			if p.get("vx", 0) < 0:
				game_node.draw_set_transform(Vector2(px + p["w"], p["y"]), 0.0, Vector2(-1, 1))
				game_node.draw_texture_rect(pimg, Rect2(0, 0, p["w"], p["h"]), false, Color(1,1,1,0.8))
				game_node.draw_set_transform(Vector2.ZERO, 0.0, Vector2.ONE)
			else:
				game_node.draw_texture_rect(pimg, Rect2(px, p["y"], p["w"], p["h"]), false, Color(1,1,1,0.8))
		else:
			game_node.draw_rect(Rect2(px, p["y"], p["w"], p["h"]), pc)
			game_node.draw_rect(Rect2(px + 4, p["y"] + 4, p["w"] - 8, p["h"] - 8), Color.WHITE)
		if GameWorld.frame % 2 == 0:
			Fighter.emit_particles(p["x"] + p["w"] / 2.0, p["y"] + p["h"] / 2.0, 3, pc, 2, 4, "circle")

# ===== 火焰区域 =====
static func _draw_flame_zones(game_node: CanvasItem, cam_x: float):
	for fz in GameWorld.flame_zones:
		var px = fz["x"] - cam_x
		if px < -50 or px > Constants.W + 50:
			continue
		if FLAME_IMG:
			game_node.draw_texture_rect(FLAME_IMG, Rect2(px, fz["y"], fz["w"], fz["h"]), false, Color(1,1,1,0.8))
		else:
			game_node.draw_rect(Rect2(px, fz["y"], fz["w"], fz["h"]), Color(1.0, 0.267, 0.0, 0.8))
