...
func draw(canvas: CanvasItem, cam_x: float = 0.0, cam_y: float = 0.0):
	...
	match type:
		"circle":
			canvas.draw_circle(Vector2(sx, sy), maxf(1.0, size), draw_color)
		"rect":
			canvas.draw_rect(Rect2(sx - size/2, sy - size/2, size, size), draw_color)
		"star":
